    ...
    <>
            ...
            {nowPlayingMovies.map((movie) => (
              <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => {setSelectedMovie(movie.id)}}>
                ...
                {selectedMovie === movie.id && (
                  <div className="absolute top-2 right-2 flex items-center justify-center bg-[color:var(--color-primary)] h-6 w-6 rounded">
      ...
      <div className="mt-6">
          ...
          <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
              ...
              <button className="bg-[color:var(--color-primary)]/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-[color:var(--color-primary)] cursor-pointer" onClick={handleDateTimeAdd}>
              Add Time
              </button>
          </div>
      </div>
            ...
            {Object.entries(dateTimeSelection).map(([date, times]) => (
              <li key={date}>
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                    {times.map((time) => (
                      <div key={time} className="border border-[color:var(--color-primary)] px-2 py-1 flex items-center rounded">
                        <span>{time}</span>
      ...
      <button className="bg-[color:var(--color-primary)] text-white px-8 py-2 mt-6 rounded hover:bg-[color:var(--color-primary)]/90 transition-all cursor-pointer">
          Add Show
      </button>
    </>
  ) : <Loading />
